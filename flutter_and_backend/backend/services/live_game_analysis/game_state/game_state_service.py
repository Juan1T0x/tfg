...
def get_all_game_states() -> Dict[str, Dict[str, Any]]:
    """
    Devuelve un dict cuyos keys son los *slugs* de cada partida
    y cuyo valor es el contenido completo de su `game_state.json`.
    """
    result: Dict[str, Dict[str, Any]] = {}
    for match_dir in _HISTORY.iterdir():
        fp = match_dir / "game_state.json"
        if fp.is_file():
            result[match_dir.name] = json.loads(fp.read_text(encoding="utf-8"))
    return result
