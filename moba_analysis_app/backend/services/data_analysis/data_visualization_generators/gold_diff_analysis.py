...
def _series_pair(frames: List[Dict], blue: int, red: int) -> Tuple[List[float], List[float]]:
    xs, ys = [], []
    for f in frames:
...
def _series_team(frames: List[Dict]) -> Tuple[List[float], List[float]]:
    ...
    for f in frames:
        t = f["timestamp"] / 60_000
        blue = sum(p["totalGold"] for pid, p in f["participantFrames"].items() if int(pid) <= 5)
        red = sum(p["totalGold"] for pid, p in f["participantFrames"].items() if int(pid) > 5)
        xs.append(t)
        ys.append(blue - red)
    return xs, ys
...
def _plot(xs: List[float], ys: List[float], title: str) -> go.Figure:
    ...
    fig.update_layout(
        title=title,
        xaxis_title="Minuto",
        yaxis_title="Diferencia de oro (azul – rojo)",
...
def generate_gold_diff(timeline_path: str | Path, output_dir: str | Path) -> None:
    ...
    pair_dirs = {name: output_dir / name for *_, name in _PAIR_IDS}
    team_dir = output_dir / "team"
    for d in (*pair_dirs.values(), team_dir):
        d.mkdir(parents=True, exist_ok=True)

    for blue, red, tag in _PAIR_IDS:
        xs_all, ys_all = _series_pair(frames, blue, red)
        if not xs_all:
            continue
        for start, end, lbl in _WINDOWS:
            ...
            if not xs:
                continue
            fig = _plot(xs, ys, f"{tag} – diff oro {lbl}")
    ...
    for start, end, lbl in _WINDOWS:
        ...
        if not xs:
            continue
        fig = _plot(xs, ys, f"Equipo – diff oro {lbl}")
        base = team_dir / f"team_{lbl}".replace(":", "")
        fig.write_html(base.with_suffix(".html"))
        fig.write_image(base.with_suffix(".png"), scale=2)

    print(f"Gráficas de oro generadas en: {output_dir.resolve()}")
